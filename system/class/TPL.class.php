...
class TPL {
	...
	static private $compileDir = CACHETPLDIR;
	...
	static private $parseAllTime = false;
	...
	static private function loops($source){
	    ...
	    while(($pos=strpos($source, "{loop ", $pos))!==false){
	        ...
	        if($endPos===false) {
	        	throw new Exception(E_USER_ERROR, "Loop does not have end mark '}'", self::$tplFile, ($pos==0?0:substr_count($source, PHP_EOL, 0, $pos)+1), null);
	        }
	...
	static private function codes($source){
	    ...
	    while(($pos=strpos($source, "{code ", $pos))!==false){
	        ...
	        // if end loop mark not found
	        if($endPos===false) {
	        	throw new Exception(E_USER_ERROR, "Code does not have end mark '}' ", self::$tplFile, ($pos==0?0:substr_count($source, PHP_EOL, 0, $pos)+1), null);
	        }
	...
	// Parse template file
	static private function parse_file($tpl_file){
		...
		if(!file_exists(self::$tplFile) && !file_exists(self::$compileFile)){
		    //throw new Exception(E_USER_ERROR, "There is no template file - '".self::$tplFile."'.", __FILE__, __LINE__, null);
		    trigger_error("There is no template file - '".self::$tplFile."'.");
		}
		...
		if(!file_exists(self::$compileFile)){
			//throw new Exception(E_USER_ERROR, "Error compiling template file - '".self::$tplFile."' in a directory '".self::$compileDir."'.", __FILE__, __LINE__, null);
			trigger_error("Error compiling template file - '".self::$tplFile."' in a directory '".self::$compileDir."'.");
		}
	...
	static public function parse($tpl){
		include self::parse_file($tpl);
