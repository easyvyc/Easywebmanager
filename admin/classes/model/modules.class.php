...
class model_modules extends model {
	...
	function list_all_modules(){
		return $this->module->listModules();	
	}        
    ...
    function list_record_table(){
        return $this->db->select(Config::$val['pr_code'] . "_record")->result_array();
    }
    
    function remove_record_item($id){
        $this->db->delete(Config::$val['pr_code'] . "_record")->where("id=:id")->bind('id', $id)->query();
    }
