<div id="id_{elm.name}" class="formElementsField {elm.style}">
	...
	<div style="float:left;min-width:150px;">
			...
			<a id="_FILEUPLOAD_FRM_{elm.column_name}_{form_elements.id.value}" href="{uploadurl}{elm.value}">{elm.value}</a>
			<br />
			
			<input type="hidden" name="old_{elm.column_name}" value="{elm.value}" >
			<input type="hidden" name="{elm.column_name}" lass="FRM" id="_FRM_{form_elements.id.value}edititem___{elm.column_name}___{form_elements.id.value}" value="" />
			...
			<script type="text/javascript">
				...
				var settings = {
					flash_url : "{config.site_url}js/swfupload/swfupload.swf",
					...
					upload_success_handler : function(file_object, server_data, response){ 
													$('#_FILEUPLOAD_FRM_{elm.column_name}_{form_elements.id.value}').attr('href', "ajax.php?content=call&module={form_data.module}&method=loadTempUploadFile_image&params[column]={elm.column_name}&params[id]={form_elements.id.value}&lng={form_data.language}&_="+Math.floor(Math.random()*10001));
													$('#_FILEUPLOAD_FRM_{elm.column_name}_{form_elements.id.value}').html(response);
