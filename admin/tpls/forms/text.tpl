<div id="id_{elm.name}" class="formElementsField {elm.style}" {block elm.extra_block_style}style="{elm.extra_block_style}"{-block elm.extra_block_style}>
	...
	<div class="e">
		<input type='text' name='{elm.name}' id="ELMID_{elm.column_name}" value='{elm.value}' class='FRM {style}_{elm.elm_type}' {block elm.editable no}readonly{-block elm.editable no} {elm.extra_params} >
	</div>
</div>
