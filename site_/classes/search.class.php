...
class search extends basic {
	...
	/*function getResultsFromPages($key){
		...
		$sql = "SELECT P.id, T.text AS description, P.parent_id, P.title, P.page_url FROM ".Config::$val['sb_page']." P " .
				"LEFT JOIN ".Config::$val['sb_text']." T " .
				"ON (P.id=T.page_id) " .
		...
		for($i=0; $i<$n; $i++){
			...
			//pa($this->path);
			if($this->path[0]['id']==Config::$val['default_page'][$this->lng]){
				$n_res[] = $res[$i];
			}
	...
	function getResultsFromModule($key, $module){
		...
		$record = $this->main_object->create($module);
	...
	function getPath($id, $arr = array()){
		$sql = "SELECT id, parent_id, title FROM ".Config::$val['sb_page']." WHERE id=$id";
		$this->db->exec($sql, __FILE__, __LINE__);
		$row = $this->db->row();
		$arr[] = $row;
		if($row['parent_id']!=0){
